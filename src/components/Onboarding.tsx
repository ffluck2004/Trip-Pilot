...
export default function Onboarding({ onSuccess }: OnboardingProps) {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    const body = isLogin 
      ? { email, password } 
      : { email, password, name, preferences: { styles: selectedStyles, interests: selectedInterests } };
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Authentication issue triggered.");
      }
      ...
      if (isLogin) {
        ...
        // Success register -> Move to preference onboarding if not filled
        setStep("preferences");
        // Save temporary registered user data
  ...
  const savePreferences = async () => {
    ...
    try {
      const res = await fetch(`/api/auth/profile/${tempUser.id}/preferences`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          preferences: {
      ...
      if (!res.ok) throw new Error("Could not persist preferences.");
