...
export default function AuthScreen({ onSuccess, onBypass }: AuthScreenProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const body = isLogin 
      ? { email, password } 
      : { email, password, name, preferences: { styles: ["Adventure", "Photography"], interests: ["Nature", "Heritage"] } };
    ...
    try {
      ...
      // Save token to localStorage to mock persistent JWT session
      localStorage.setItem("trippilot_jwt", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.demo-payload");
